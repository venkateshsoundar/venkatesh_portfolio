...
.section-title { font-size: 1.6rem; border-bottom: 3px solid #4ECDC4; margin-bottom: 12px;
                  color: #4ECDC4; padding-bottom: 4px; }
...
.project-item img { width: 100%; aspect-ratio: 1 / 1; object-fit: cover; border-radius: 8px;
                     transition: transform .3s ease; }
.project-item:hover img { transform: scale(1.1); filter: brightness(1.1); }
.overlay { position: absolute; top: 0; left: 0; width: 100%; height: 100%;
           background: rgba(78,205,196,0.7); display: flex; align-items: center;
           justify-content: center; color: #121212; opacity: 0;
           transition: opacity .3s ease; font-size: 1.2rem; text-align: center;
           padding: 10px; }
...
with col_main:
    ...
    sections = [
        ...
        ("Experience", "<ul>" + "".join(f"<li>{e}</li>" for e in [
            ...
            "Agile Team Lead", "Insurance & Healthcare Risk Analytics"
        ]) + "</ul>"),
        ("Skills", "<ul>" + "".join(f"<li>{s}</li>" for s in [
            "Python, SQL, R", "AWS (S3, EC2, Lambda, SageMaker)",
            "Streamlit, Tableau, Power BI", "Scikit-learn, OpenCV, Flask", "Git, Jira, Agile"
        ]) + "</ul>"),
