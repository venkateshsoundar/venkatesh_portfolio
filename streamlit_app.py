...
# --- Load resume bullets ---import streamlit as st
from openai import OpenAI
import requests
import io
import PyPDF2
...
.card { display:block!important; width:100%!important; border-radius:12px; padding:20px; margin-bottom:20px; background:linear-gradient(135deg,#1F2A44 0%,#324665 100%); transition:transform .3s ease,box-shadow .3s ease; color:#ffffff; box-sizing:border-box; }
.card * { color:#ffffff!important; }
.hover-zoom { transition:transform .3s ease; }
.hover-zoom:hover { transform:translateY(-5px); }
.section-title { font-size:1.6rem; font-weight:bold; margin-bottom:12px; padding-bottom:4px; color:#ffffff!important; text-decoration:none!important; }
...
.overlay { position:absolute; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); display:flex; align-items:center; justify-content:center; opacity:0; transition:opacity .3s ease; font-size:1.2rem; text-align:center; padding:10px; color:#ffffff!important; }
